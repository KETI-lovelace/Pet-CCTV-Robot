...
void kinesis_video_init(CustomData* data) {
    ...
    printf("%s streaming region : %s\n", LOG_INFO, defaultRegionStr.c_str());
...
int gstreamer_init(int argc, char* argv[], CustomData* data) {
    ...
    printf("%s finish gstreamer src init\n", LOG_STATUS);
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
        printf("%s usage : ./KinesisStreaming <streaming name>\n", LOG_ERROR);
        return 1;
    }
    ...
    printf("%s streaming name : %s\n", LOG_INFO, data.stream_name);

    /* init Kinesis Video */
    try {
        kinesis_video_init(&data);
        printf("%s finish kinesis video init\n", LOG_STATUS);
        kinesis_video_stream_init(&data);
        printf("%s finish kinesis video stream init\n", LOG_STATUS);
    }
    catch (runtime_error& err) {
        printf("%s runtime error occured : \n%s\n", LOG_ERROR, err.what());
        return 1;
    }
